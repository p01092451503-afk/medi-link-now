...
const VoiceRecorder = ({ onTranscript, onParsedData }: VoiceRecorderProps) => {
  ...
  useEffect(() => {
    ...
    recognitionInstance.onresult = (event: any) => {
      ...
      if (finalTranscript) {
        ...
        setIsEditMode(true); // Show edit mode after voice recognition
        recognition.stop();
  ...
  }, [editedTranscript, onTranscript, parseWithAI]);
      ...
      <AnimatePresence mode="wait">
        {isEditMode && !isProcessingAI ? (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="border-2 border-primary/30 rounded-2xl p-4 bg-primary/5"
          >
            <div className="flex items-center gap-2 mb-3">
              <Edit3 className="w-4 h-4 text-primary" />
              <span className="text-sm font-medium text-primary">인식된 텍스트 확인 및 수정</span>
            </div>
            ...
            <p className="text-xs text-muted-foreground mt-2 text-center">
              텍스트를 수정한 후 "AI 분석 시작"을 눌러주세요
            </p>
          </motion.div>
        ) : (
          <motion.div key="mic-mode">
            {/* Mic Button */}
            <motion.button
