import { Locate, Check, MapPin, Navigation, Shield } from "lucide-react";
...
const infoItems = [
  { icon: Navigation, text: "내 주변 응급실·약국 검색" },
  { icon: MapPin, text: "최단거리 경로 안내" },
  { icon: Shield, text: "서버에 저장되지 않음" },
];
...
const StepLocation = ({ granted, onRequest }: StepLocationProps) => (
  <div className="flex flex-col items-center text-center gap-6 px-6">
    ...
    >
      <AnimatePresence mode="wait">
        {granted ? (
          ...
          <motion.div key="locate" initial={{ scale: 0.8 }} animate={{ scale: 1 }}>
            <Locate className="w-8 h-8 text-foreground" />
          </motion.div>
        )}
      </AnimatePresence>
    </motion.div>
    ...
    <div>
      ...
      <p className="text-[13px] text-muted-foreground mt-1">내 주변 응급실을 찾기 위해 필요합니다</p>
    </div>
    ...
    <AnimatePresence mode="wait">
      ...
      {granted === true && (
        ...
        >
          ...
          <span className="text-[13px] font-medium text-foreground">위치 권한이 허용되었습니다</span>
        </motion.div>
      )}
      {granted === false && (
        <motion.div
          ...
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="text-[12px] text-muted-foreground max-w-xs leading-relaxed"
        >
          위치 권한이 거부되었습니다.
          <br />
          <span className="text-[11px]">설정에서 수동으로 허용하거나, 건너뛰어도 지역을 검색할 수 있습니다.</span>
        </motion.div>
      )}
    </AnimatePresence>
  </div>
